...
describe('Open test.md in viewer', function() {
	...
	it('Shares the file as a public link with write permissions', function () {
		...
		cy.get('#app-sidebar-vue .sharing-link-list .action-checkbox input[type=checkbox]').first().check({ force: true })
		cy.get('#app-sidebar-vue .sharing-link-list .action-checkbox input[type=checkbox]', { timeout: 4000 }).first().should('be.checked')
		cy.get('#app-sidebar-vue a.sharing-entry__copy')
			.should('have.attr', 'href').and('include', '/s/')
			.then((href) => {
				cy.visit(href)
